...
|------------|----------------------|
| `prefers-reduced-motion: reduce` | All durations route through the `--wel-motion` multiplier token, set to `0` under the preference ([05-design-tokens.md](05-design-tokens.md)). No component writes a raw duration. Scroll-driven and view-transition effects are additionally gated off — a timeline-driven animation has no duration for the multiplier to zero, so the media gate is the mechanism: the carousel progress bar, the `.reveal` and `.parallax` image utilities (`.reveal`'s hidden start state and `.parallax`'s headroom scale exist only inside their gates, so reduced-motion users never meet a blank or a zoomed crop), and the view-transitions module with its `data-vt-image` morphs ([image-fx spec](components/image-fx.md)). Spatial hover motion gates off wholesale too: `.ken-burns` and `.tilt` sit entirely inside the preference — a slow pan/zoom or a rotation snap has no non-vestibular reduced equivalent (zeroing their durations would *crop or jump*, not calm), so the media simply never moves. Opacity-only fades may remain (non-vestibular) — `.color-reveal`'s filter transition rides the multiplier and collapses to an instant swap. |
...
| `prefers-reduced-transparency: reduce` | Any translucent surface treatment falls back to an opaque equivalent; no shipped token pairing may rely on transparency to meet contrast. First occupant: the `.frosted-caption` bar drops its blur and goes fully opaque `--wel-color-surface` ([image-fx spec](components/image-fx.md)). |
