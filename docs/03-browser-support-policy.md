...
| Scroll-driven animations (`animation-timeline`) | Scroll progress bars, reveal-on-scroll effects (`.reveal` image utility), `.parallax` image drift | Effects simply absent; content static and fully readable. The `.reveal` hidden start state exists **only inside** the `@supports` gate, so no engine ever holds content at `opacity: 0`; `.parallax` keeps its headroom scale inside the same gate, so the fallback image is unscaled and motionless |
| `mask-image` / `mask-composite` | `.edge-fade` image bleeds and vignettes | Unfaded image — additive by nature, ships ungated |
| `backdrop-filter` | `.frosted-caption` glass caption bars (blur+saturate over a translucent scrim) | Ungated near-solid surface scrim — caption text stays readable over any image; only the glass look is absent |
| `clip-path: shape()` | `.organic-frame` blob/arch/scallop image crops | Ungated `border-radius` approximations (blob/arch analogues, plain rounded crop for scallop); the fallback radius is zeroed inside the `@supports` gate so the clips never intersect |
| `corner-shape` | `.squircle` superellipse corners | Plain round corners at the same radius — `corner-shape` only reshapes how `border-radius` corners draw, so its absence is inherently additive; ships ungated |
| `object-view-box` | `.view-crop` in-source art-direction crops | Full uncropped image — engines without it drop the unknown property at parse time, and the paired ungated `object-fit: cover` is a no-op at the image's natural ratio |
